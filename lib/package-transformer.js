...
function transformDependency(d, file, rootDir, newPprojectRootDir) {
    ...
    function resolvePath(path) {
        ...
        if (fs.existsSync(absolutePath)) {
            if (newPprojectRootDir) {
                return nodePath.relative(newPprojectRootDir, absolutePath);
            } else {
                var relPath = nodePath.relative(dir, absolutePath);
                if (relPath.charAt(0) !== '.') {
                    relPath = './' + relPath;
                }
                return relPath;
...
function transform(pkg, file, rootDir, newPprojectRootDir) {
    ...
    if (dependencies) {
        dependencies.forEach(function(d) {
            d = transformDependency(d, file, rootDir, newPprojectRootDir);
            if (d) {
